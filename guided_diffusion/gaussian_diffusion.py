"""
Modified gaussian_diffusion.py with UniDepth depth integration.

Key modification: At each sampling step, replace the depth channel with UniDepth depth
- For conditioning (phi learning): use clean UniDepth depth in x_0_hat
- For next step input: use noisy UniDepth depth in img

This ensures phi parameters are learned based on UniDepth depth throughout the process.
"""
...
class GaussianDiffusion:
    ...
    def p_sample_loop(self,
                      ...
                      sample_pattern=None,
                      # =====================================================
                      # NEW: UniDepth parameters
                      # =====================================================
                      unidepth_depth=None,  # Clean UniDepth depth [1, 1, H, W], normalized to [-1, 1]
                      # =====================================================
                      **kwargs):
        """
        The function used for sampling from noise.
        
        NEW: If unidepth_depth is provided, the depth channel will be replaced
        with UniDepth depth at every timestep for consistent physics-guided restoration.
        """
        ...
        # =====================================================
        # NEW: Log UniDepth status
        # =====================================================
        if unidepth_depth is not None:
            print(f"[UniDepth] Depth guidance enabled. Shape: {unidepth_depth.shape}")
        # =====================================================
        ...
        for idx in pbar:
            ...
            for alternate_ii in range(alternate_len):
                ...
                if pretrain_model == 'osmosis' and not rgb_guidance:
                    ...
                    # =====================================================
                    # NEW: Replace depth in pred_xstart with clean UniDepth
                    # This ensures phi learning uses UniDepth depth
                    # =====================================================
                    if unidepth_depth is not None:
                        # Clone to avoid modifying original
                        out['pred_xstart'] = out['pred_xstart'].clone()
                        # Replace depth channel (index 3) with clean UniDepth
                        out['pred_xstart'][:, 3:4, :, :] = unidepth_depth.to(device)
                    # =====================================================
                    ...
                    # =====================================================
                    # NEW: Replace depth channel in img with noisy UniDepth
                    # This ensures next step receives UniDepth-based depth
                    # =====================================================
                    if unidepth_depth is not None and idx > 0:
                        # Get noisy version of UniDepth at timestep t-1 (next step's input)
                        # Note: idx is current timestep, next step will be idx-1
                        next_time = torch.tensor([idx - 1] * img.shape[0], device=device)
                        noisy_unidepth = self.q_sample(unidepth_depth.to(device), t=next_time)
                        # Replace depth channel
                        img[:, 3:4, :, :] = noisy_unidepth
                    elif unidepth_depth is not None and idx == 0:
                        # At final step, use clean UniDepth
                        img[:, 3:4, :, :] = unidepth_depth.to(device)
                    # =====================================================
        ...
        # =====================================================
        # NEW: Ensure final output has UniDepth depth
        # =====================================================
        if unidepth_depth is not None and pretrain_model == 'osmosis' and not rgb_guidance:
            out['pred_xstart'][:, 3:4, :, :] = unidepth_depth.to(device)
        # =====================================================
